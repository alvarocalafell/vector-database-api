...
def test_create_library(api_client, vector_db, sample_library):
...
def test_get_library(api_client, vector_db, sample_library):
    api_client.post("/libraries/", json=sample_library)
    response = api_client.get(f"/libraries/{sample_library['id']}")
    assert response.status_code == 200
    assert response.json() == sample_library


def test_update_library_metadata(api_client, vector_db, sample_library):
    ...
    response = api_client.put(f"/libraries/{sample_library['id']}", json=updated_library)
...
def test_delete_library(api_client, vector_db, sample_library):
    api_client.post("/libraries/", json=sample_library)
    response = api_client.delete(f"/libraries/{sample_library['id']}")
    assert response.status_code == 200
    response = api_client.get(f"/libraries/{sample_library['id']}")
    assert response.status_code == 404

def test_create_existing_library(api_client, vector_db, sample_library):
    api_client.post("/libraries/", json=sample_library)
    response = api_client.post("/libraries/", json=sample_library)
    assert response.status_code == 400

def test_get_nonexistent_library(api_client, vector_db):
    response = api_client.get("/libraries/nonexistent")
    assert response.status_code == 404

def test_update_nonexistent_library(api_client, vector_db, sample_library):
    # Test updating a non-existent library
    nonexistent_id = "nonexistent"
    updated_library = {
        "metadata": sample_library["metadata"]
    }
    response = api_client.put(f"/libraries/{nonexistent_id}", json=updated_library)
    ...
    response = api_client.put(f"/libraries/{existing_library_id}", json=mismatched_library)
...
def test_delete_nonexistent_library(api_client, vector_db):
    response = api_client.delete("/libraries/nonexistent")
    assert response.status_code == 404