...
def test_add_document(api_client, vector_db, sample_library, sample_document):
...
def test_get_document(api_client, vector_db, sample_library, sample_document):
    ...
    response = api_client.get(f"/documents/{sample_library['id']}/{sample_document['id']}")
...
def test_update_document(api_client, vector_db, sample_library, sample_document):
...
def test_delete_document(api_client, vector_db, sample_library, sample_document):
...
def test_add_document_to_nonexistent_library(api_client, vector_db, sample_document):
    logger.debug("Starting test_add_document_to_nonexistent_library")
    
    response = api_client.post("/documents/nonexistent", json=sample_document)
    assert response.status_code == 404, f"Expected 404, got: {response.status_code}"
    
    logger.debug("Finished test_add_document_to_nonexistent_library")

def test_get_document_from_nonexistent_library(api_client, vector_db):
    logger.debug("Starting test_get_document_from_nonexistent_library")
    
    response = api_client.get("/documents/nonexistent/doc_id")
    assert response.status_code == 404, f"Expected 404, got: {response.status_code}"
    
    logger.debug("Finished test_get_document_from_nonexistent_library")

def test_update_document_in_nonexistent_library(api_client, vector_db, sample_document):
    ...
    response = api_client.put(f"/documents/{nonexistent_library_id}/{document_id}", json=sample_document)
...
def test_delete_document_from_nonexistent_library(api_client, vector_db):
    logger.debug("Starting test_delete_document_from_nonexistent_library")
    
    response = api_client.delete("/documents/nonexistent/doc_id")
    assert response.status_code == 404, f"Expected 404, got: {response.status_code}"
